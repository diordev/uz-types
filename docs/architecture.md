...
- `Pinfl::parse_strict()` checksum, jins/asr belgisi hamda kun va oy diapazonini qo'shib tekshiradi;
- `PhoneNumber::parse_strict()` operator yoki hudud kodining joriy ro'yxatda borligini tekshiradi;
...
checksum, 1-raqamdagi jins/asr semantikasi hamda kun `1..=31` va oy `1..=12` oralig'ini
tekshiradi; u `31.02` kabi to'liq kalendar sanasini aniqlamaydi. Kalendar to'g'riligini
`date` feature'i yoqilgandagi `Pinfl::birth_date()` yoki deterministik
`birth_date_at()` tekshiradi. `MOBILE_CODES`, checksum va jins/asr kabi faktlarni
`parse()` ichiga ko'chirish DB va event replay'ini buzishi mumkin.
...
sirt va xulq o'zgarmasa SemVer bo'yicha mos bo'ladi. To'liq 13 source fayli xaritasi §10 da.
...
`parse()`ni, keyin o'z qo'shimcha tekshiruvini chaqiradi.
...
Jonli DB testi yo'q; mavjud testlar faqat compile-time trait boundlarini tekshiradi.
...
| `PhoneNumber` | Boshlang'ich `+` va bo'shliq, `-`, `(`, `)`, `.` ajratgichlari olinadi; 12 raqam va `998` prefiksi | `parse_strict()`, `operator_code()`, `subscriber_number()`, `is_mobile()`, `is_known_operator()`, `to_international()` | `DIGIT_LEN = 12`, `COUNTRY_CODE = "998"`, `OPERATOR_CODE_LEN = 2`, `MOBILE_CODES`, `REGIONAL_CODES = 60..=79` |
...
`MOBILE_CODES: &[&str]` slice, `REGIONAL_CODES: RangeInclusive<u8>` esa `60..=79`.
Bu shakllarda yangi
operator kodi qo'shilishi massiv uzunligini public tipga kiritmaydi va API'ni keraksiz
breaking o'zgarishdan saqlaydi.
...
dev-dependency'larni, jumladan Rust 1.86 talab qiladigan Criterion benchmarkini tortadi.
1.94/sqlx tekshiruvida esa `--all-targets --all-features` ataylab bor.
...
| PINFL checksum formulasi `[7,3,1] mod 10` bilan mos | `pinfl_checksum_generator_agrees`: `[1-6][0-9]{12}` body | Rasmiy registry, to'liq kalendar sana va `parse_strict`ning barcha kombinatsiyasi emas |
| `NumId` ikki reprida parse/display roundtrip va DB-safe bog'lanishi | [props.rs](../tests/props.rs): `\\PC{0,32}` va barcha `u64`; [id.rs](../src/id.rs) unit testlari | `parse_db_safe`ning aniq chegara misollari property emas, `db_safe_bounds_move_the_error_to_construction` unit testida; jonli DB yo'q |
...
| Serde smart constructorni chetlab o'tmaydi | [serde.rs](../tests/serde.rs): Passport roundtrip, Phone normalizatsiyasi, Passport/Pinfl/Email/AccessToken invalid holatlari | Har tipning har xato varianti va har deserializer formati qamralmagan |
...
| SQLx Postgres traitlari va ichki tip mappinglari mavjud | [sqlx_bounds.rs](../tests/sqlx_bounds.rs) barcha qo'llab-quvvatlangan nonsir tiplarni compile-time tekshiradi | Jonli encode/decode/query yo'q; `Vec<T>` qamrovi faqat `Passport` va `NumId<Tag, i64>` |
...
**To'liq tekshiruv.** `just ci` lokal recipe'larni CI yo'nalishlariga mos ketma-ket bajaradi:
...
4. Toza working tree'da `just publish-check` bilan dry-run bajaring.
5. Faqat aniq reliz qaroridan keyin `just publish` bajaring; publish qaytarilmaydi.
...
Release tayyorgarligidagi package versiyasi `0.22.0`, Rust editioni 2024. Manifestdagi
umumiy `rust-version` 1.85; `sqlx` bilan amaliy pol §6 da ko'rsatilganidek 1.94.

### 13 source fayli xaritasi
...
Qo'shimcha bajariladigan artefaktlar: `tests/`dagi property, Serde, SQLx va compile-fail
integration testlari; [parse.rs](../benches/parse.rs) benchmarki;
